...
import {
  elRiesgoDeNoArriesgar,
  riesgoArriesgarSubtitle,
} from '@/lib/el-riesgo-de-no-arriesgar';
import { LangArticle, type Lang } from '@/components/pages/lang-article';

function RiesgoArriesgarBody({ lang }: { lang: Lang }) {
  return (
    <>
      {/* Subtítulo del ensayo */}
      <p className="mt-10 text-center font-serif text-xl italic text-muted-foreground text-balance">
        {riesgoArriesgarSubtitle[lang]}
      </p>

      {/* Cuerpo del ensayo */}
      <article className="mt-10 space-y-6">
        {elRiesgoDeNoArriesgar.map((block, i) => {
          if (block.type === 'image') {
            return (
...
export default function RiesgoArriesgarEssay() {
  return (
    <div className="max-w-3xl mx-auto">
      <LangArticle
        es={<RiesgoArriesgarBody lang="es" />}
        en={<RiesgoArriesgarBody lang="en" />}
      />
    </div>
  );
}
